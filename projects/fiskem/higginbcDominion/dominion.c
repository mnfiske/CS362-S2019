			...
			if (i != currentPlayer)
			{
				if ( state->handCount[i] > 4 )
		      			...
		      			while( state->handCount[i] > 0 )
					{
		      			...
		      			for (int j = 0; j < 4; j++)
					{
			  			drawCard(j, state); //bug - should pass i, not j
					}
		    		}
			}
	    	}
...
int salvagerEffect(struct gameState* state, int handPos, int choice1)
{
	int currentPlayer = whoseTurn(state);
...
int seahagEffect(struct gameState* state)
{
	int currentPlayer = whoseTurn(state);
      	...
      	for (int i = 0; i < state->numPlayers; i++)
	{
		if (i == currentPlayer)	// bug - should be != instead of ==
		{
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case salvager:
	return salvagerEffect(state, handPos, choice1);
