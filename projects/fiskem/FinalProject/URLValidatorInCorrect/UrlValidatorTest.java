...
protected void setUp() {
   ...
   public void testRandomIsValid() {
       ...
       testRandomIsValid(testUrlPartsOptions, options);
  }

  public void testRandomIsValidScheme() {
     if (printStatus) {
        System.out.print("\n testRandomIsValidScheme() ");
     }
  ...
  /**
   * Create set of tests by taking the testUrlXXX arrays, randomly picking elements from them, then
   * validating permutations of their combinations.
   *
  ...
  public void testRandomIsValid(Object[] testObjects, long options) {
     ...
     for (int i = 0; i < 100000; i++) { //100000 iterations of randomized input test
         StringBuilder testBuffer = new StringBuilder();
        boolean expected = true;
        for (int testPartsIndexIndex = 0; testPartsIndexIndex < testPartsIndex.length; ++testPartsIndexIndex) { //testPartsIndex.length = 5
       	ResultPair[] part = (ResultPair[]) testObjects[testPartsIndexIndex];
           int randomizedindex = (int) (Math.random() * part.length); //picks a random element from one of the ResultPair arrays, value between 0 and size of array - 1
           testBuffer.append(part[randomizedindex].item);
           expected &= part[randomizedindex].valid;
        }
